...
运动学层:
    ...
    [F5] 姿态误差世界系计算, 双覆盖最短路径
    分通道限幅 [FIX-2]: |v|<=0.6, |w|<=1.0 (参考生成器, 定理外生输入)
    主任务: 自适应阻尼 DLS; 零空间: 可操作度梯度 + [B] 限位排斥/刹车
    dq_des 经 Slew 限变化率 (定理假设P3: 参考Lipschitz)
动力学层: USE_PPC 切换 PPC / DSC
  [A+] PPF 变换为纯微分同胚, 无任何 clip:
       前提监测 + 事件触发重初始化 (q_d<-q / alpha_f<-dq), 事件全记录
  [S1] gamma1=0.4, rho1_inf 加宽 (线性区裕度 G1<=filter/4)
  [S3] ssign delta=0.5, epsilon=0.15 (设计光滑化, 非 clip)
底盘层:   [FIX-1] 力分配 或 [FIX-3] 轮速伺服; 输出 4轮+7臂=11 维
安全层:   力矩按 ctrlrange 硬限幅(执行器饱和, 归入扰动d) / PD settling / NaN守卫
诊断:     dFRM / w∥ / jlim / 重初始化事件流 / 终局违规报告
========================================================
"""
import os
import numpy as np
...
class PPFVector:
    ...
    def domain_ok(self, e, t):
        """定理前提: e 严格在开定义域 |e| < eps_up*rho(t) 内"""
        return np.abs(e) < self.eps_up * self.f(t)

    def transform(self, e, t):
        """[A+] mu = ln( eps_up*(phi-eps_lo) / (-eps_lo*(eps_up-phi)) )
           在开定义域内为严格微分同胚, 边界排斥增益 -> 无穷.
           不做任何 clip —— 定义域外的求值由前提监测阻止 (见 compute_ctrl)."""
        ft = self.f(t)
        phi = e / ft
        return np.log((self.eps_up * (phi - self.eps_lo)) /
                      (-self.eps_lo * (self.eps_up - phi)))

def ssign(x, delta=0.5):
...
# ============================================================
# 2b. Slew 限变化器 (定理假设P3: 参考Lipschitz 的实现)
# ============================================================
class SlewLimiter:
...
class MobileManipulatorPPCController:
    ...
    def __init__(self, model, arm_joint_names,
        ...
        self.reinit_events = []          # [A+] 事件流: (t, channels, level, max|phi|)
    ...
    def reset_states(self, data, dq_des=None):
        q = self._get_q(data)
        self.q_d = q.copy()              # (P1) e1(0)=0 严格成立
        ...
        # (P2) 显式检查: e2(0)=dq(0) 必须在漏斗2开定义域内
        dq0 = np.array([data.qvel[a] for a in self.all_dof_adrs])
        lim0 = 0.9 * self.ppf2.f(0.0)
        bad = np.where(np.abs(dq0) >= lim0)[0]
        if len(bad):
            print(f'[WARN] 前提P2违例 @handover: |dq0|={np.abs(dq0[bad])} '
                  f'>= 0.9*rho2(0)={lim0[bad]}, channels={bad.tolist()}')
        else:
            print(f'[PREMISE] P1: e1(0)=0 in D0 (exact) | '
                  f'P2: max|dq0|={np.max(np.abs(dq0)):.3f} < '
                  f'min 0.9*rho2(0)={np.min(lim0):.3f}  OK')
    ...
    def compute_ctrl(self, data, dt):
        ...
        # ---- [A+] 前提监测 Level-1: 定义域外 => 事件触发重初始化 ----
        #   不是 clip: 变换保持纯映射; 投影 q_d<-q 使 e1=0, 前提(P1)严格恢复
        ...
        if not np.all(ok1):
            ...
            print(f'[REINIT] t={self.t:.2f}s L1 channels={v.tolist()} '
                  f'max|phi|={phi[v].max():.3f} (>=0.9) -> q_d<-q')
            self.q_d = q.copy()
            e1 = np.zeros_like(e1)
        ...
        if not np.all(ok2):
            ...
            print(f'[REINIT] t={self.t:.2f}s L2 channels={v.tolist()} '
                  f'max|phi|={phi[v].max():.3f} (>=0.9) -> alpha_f<-dq')
        ...
        return np.clip(tau, self.ctrl_lo, self.ctrl_hi)   # 执行器饱和(物理)
...
def plot_results(rec, ctrl):
    ...
    fig1.suptitle('Level-1 Errors inside Prescribed Performance Funnels '
                  '(pure transform, no clipping)', fontsize=14)
    fig1.tight_layout()
    ...
    a3[3].set_yscale('log')
    a3[3].set_title('PPF transform (pure, unbounded)')
    a3[3].legend(fontsize=8)
...
def main():
    ...
    # ---- [C] 重初始化事件流 + 终局违规报告 ----
    print(f'\n[REPORT] re-initialization events: {len(ctrl.reinit_events)}')
    if not ctrl.reinit_events:
        print('   none -- 漏斗保证全程未被打破, 定理前提 (P1)(P2) 全程成立')
    else:
        for (te, chs, lv, ph) in ctrl.reinit_events[:20]:
            print(f'   t={te:6.2f}s  level-{lv}  channels={chs.tolist()}  '
                  f'max|phi|={ph:.3f}')
