...
运动学层:
    ...
    [F5] 姿态误差改为自包含世界系计算(不依赖 mju_subQuat 的框架约定),
         双覆盖取最短路径; 保留 subQuat 对照 dFRM 用于确认旧 bug
    分通道限幅 [FIX-2]: |v|<=0.6, |w|<=1.0 (映射前, 通道间不污染)
    主任务:  自适应阻尼 DLS
    零空间:  可操作度梯度上升, 瞬态门控 (|e_p|>0.5m 冻结)
    dq_des 经 Slew 限变化率
动力学层: USE_PPC 切换 PPC / DSC
  [S1] gamma1=0.4, rho1_inf 加宽 (G1 <= filter/4 裕度准则)
  [S2] mu clip ±1.0   [S3] ssign delta=0.5, epsilon=0.15
底盘层:   [FIX-1] 力分配 T_force  或  [FIX-3] 轮速伺服; 输出 4轮+7臂=11 维
安全层:   力矩按 ctrlrange 硬限幅 / Kb 封顶 / PD settling
诊断:     dFRM(subQuat框架差) / w∥(误差轴上的实际角速度) / jlim(关节限位裕度)
========================================================
"""
import os
import numpy as np
...
# ============================================================
# 1b. [F5] 世界系姿态误差 (自包含, 不依赖 mju_subQuat 约定)
# ============================================================
def orientation_error_world(tq, qc):
    """世界系旋转向量 (axis*angle, angle∈[0,pi]), 把 qc 转到 tq:
       qe = tq ⊗ qc*  (左乘 delta => 轴在世界系); w<0 时取负 => 最短路径."""
    qc_inv = np.zeros(4)
    mujoco.mju_negQuat(qc_inv, np.asarray(qc, float))
    qe = np.zeros(4)
    mujoco.mju_mulQuat(qe, np.asarray(tq, float), qc_inv)
    if qe[0] < 0.0:                      # 双覆盖: 同一旋转的两个四元数
        qe = -qe
    w = np.zeros(3)
...
def null_space_ik_controller(model, data, scratch, cols,
                             ...
                             with_rotation=False):
    """
    返回: dq, w, lam, |dq_null|, |e_p|, |e_r|, max|dq|,
          diag{ frame_diff, w_along, w_real_norm }
    frame_diff = ||subQuat误差 - 安全误差||: 翻转中 O(1) => 旧框架bug坐实
    w_along    = 实际手角速度在(安全)误差轴上的投影: 应为 +w_max 直到收敛
    """
    ee = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, EE_BODY_NAME)
    ...
    # ---- 诊断: 实际手角速度 (世界系, 由当前 qvel) ----
    dq_now = np.array([data.qvel[c] for c in cols])
    omega_real = J[3:] @ dq_now
    w_real_norm = float(np.linalg.norm(omega_real))
    ...
    if k_eff > 1e-6 and w > 0.3 * w_thr:
        ...
        if gn > 1e-12:
            ...
            dq_null = k_eff * (N @ g_hat)

    dq = dq_task + dq_null
    diag = {'frame_diff': frame_diff, 'w_along': w_along,
            'w_real_norm': w_real_norm}
...
def plot_results(rec, ctrl):
    ...
    for i in range(ctrl.num_ctrl):
        a = ax1[i//2, i%2]
        a.plot(t, e1[:, i], 'b', lw=1.0, label=fr'$e_1[{i+1}]$')   # 修复双下标
    ...
    fig3, a3 = plt.subplots(1, 5, figsize=(23, 3.5))
    ...
    a3[3].axhline(1.0, color='k', ls=':', lw=.8)
    a3[3].set_title('PPF transform activity'); a3[3].legend(fontsize=8)
    a3[4].plot(t, wal, 'b', lw=1.0, label=r'$\omega_\parallel$ (real, along err)')
...
def main():
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        print(f'[INFO] Handover to {"PPC" if USE_PPC else "DSC"} '
              ...
              f'| quat={TARGET_QUAT} '
              f'| out={ctrl.num_out}, nu={model.nu}, '
              f'lim={ctrl.ctrl_lo.shape[0]}')
        ...
        while viewer.is_running() and t < T_SIM:
            ...
            if k % PRT_SKIP == 0:
                jl, wj = joint_limit_margin(model, data,
                                            ctrl.arm_qpos_adrs, ARM_JOINT_IDS)
